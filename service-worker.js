...
function isLocalProviderRequest(url){const h=url.hostname.toLowerCase();return h==='localhost'||h==='127.0.0.1'||h==='[::1]'||h==='::1'||/^10\\./.test(h)||/^192\\.168\\./.test(h)||/^172\\.(1[6-9]|2\\d|3[0-1])\\./.test(h);}
self.addEventListener('fetch',event=>{const r=event.request,u=new URL(r.url);/* Never intercept local/private AI-provider requests. Chrome must enforce Local Network Access directly in the page context. */if(isLocalProviderRequest(u))return;if(r.mode==='navigate'){event.respondWith(handleNavigation(r));return;}if(u.origin===self.location.origin&&u.pathname.endsWith('.js')){event.respondWith(networkFirst(r));return;}event.respondWith(caches.match(r).then(c=>c||fetch(r)));});
